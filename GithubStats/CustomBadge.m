...
- (id) initWithString:(NSString *)badgeString withScale:(CGFloat)scale withShining:(BOOL)shining withCornerRoundness:(float)conerRoundness
{
	...
	if(self!=nil) {
		...
		self.badgeInsetColor = [UIColor colorWithRed:239.0f/255.0f green:100.0f/255.0f blue:33.0f/255.0f alpha:1.0f];
		self.badgeCornerRoundness = conerRoundness;
...
// I recommend to use the allocator customBadgeWithString
- (id) initWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining withCornerRoundness:(float)conerRoundness
{
	self = [super initWithFrame:CGRectMake(0, 0, 25, 25)];
	if(self!=nil) {
		self.contentScaleFactor = [[UIScreen mainScreen] scale];
...
{
	return [[self alloc] initWithString:badgeString withScale:1.0 withShining:NO withCornerRoundness:0.4f];
}


// Creates a Badge with a given Text, Text Color, Inset Color, Frame (YES/NO) and Frame Color 
+ (CustomBadge*) customBadgeWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining withCornerRoundness:(float)conerRoundness
{
	return [[self alloc] initWithString:badgeString withStringColor:stringColor withInsetColor:insetColor withBadgeFrame:badgeFrameYesNo withBadgeFrameColor:frameColor withScale:scale withShining:shining withCornerRoundness:conerRoundness];
}
